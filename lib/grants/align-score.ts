...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
IDENTITY GOVERNS (the #140 razor's SECOND half -- read with it)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
The #140 razor forbids UNDER-scoring a real fit. This forbids OVER-scoring a manufactured one -- the same error in the other direction, and the more common one: confirming the client is an eligible ENTITY TYPE, then backfilling a plausible role instead of asking whether the client actually PERFORMS the funded work.
- IDENTITY FIRST. Before you score, fix what this organization FUNDAMENTALLY IS from its AUTHORITATIVE identity fields -- prime_capacity.can_prime, the profile summary, and inferred/gaps: is it a FUNDER / fiscal sponsor, a direct OPERATOR / implementer, a TEACHING institution, a RESEARCH institution, a CONVENER? Those identity fields OUTRANK the capability / role / program lists. Read every capability, program, and eligibility fact THROUGH that identity, never the reverse.
- A CAPABILITY THAT CONTRADICTS IDENTITY IS RE-READ, NOT BELIEVED. A distilled profile can be internally contradictory -- it may carry a funder identity AND a capability line that reads like an implementer. When they conflict, the IDENTITY WINS: re-read the capability as what the identity supports. A conservation FOUNDATION's "habitat restoration" is habitat work it FUNDS or convenes, not field work it performs; a TEACHING college's "research" capability is instruction, not a funded R&D role; a "land steward / on-the-ground operator" line on a can_prime=FALSE funder is a manufactured role -- discount it. Never let the capability list override the identity that produced it.
- ENTITY-TYPE ELIGIBILITY IS NOT FIT, AND MAY NOT APPEAR IN THE FIT RATIONALE. "Eligible as an IHE / a 501(c)(3)" is a precondition, never evidence of alignment. A teaching college is not a fit for an R&D purpose because it is an IHE; a foundation is not a fit for field construction because it is a 501(c)(3). If your only reason to score >=2 is that the entity type is on the eligible list, the score is 1.
- REFUSE TO CONSTRUCT A FIT. Never invent eligibility exclusivity ("the only eligible entity type"), invent a partner / sub SEAT the grant does not require, insert a NON-CLIENT as the prime so the client can ride along, or REFRAME the program's funded purpose to match the client. If the only way this reaches a 2 or 3 is one of those moves, it is a NO-GO (0-1), not a conditional fit. A fit that has to be constructed is not a fit -- a clean no-go beats a rationalized 1.
- IDENTITY-FIRST CUTS ONE WAY: IT CAPS MISSION FIT, IT NEVER MANUFACTURES A DISQUALIFICATION. Everything above forbids INFLATING a score (a constructed fit, entity-eligibility dressed as alignment). It does NOT license inventing an ELIGIBILITY bar. If prime_capacity.can_prime is TRUE and the client's entity type is on this grant's eligible-entity list, the client IS an eligible Prime: do not downgrade it to Sub/Facilitator, cap its fit_score, or route it to a government prime on the SPECULATION that the grant might be prime-restricted. A real eligibility limit must be GROUNDED in the grant's stated eligible-entity list or an explicit stated restriction, never assumed from identity-first skepticism or a bare "verify against the NOFO." A can_prime=TRUE organization that genuinely PERFORMS the funded work for the funded population is a clean 2-3 on the FIT test; an unverified eligibility question belongs in before_you_approve as a STOP and MUST NOT lower fit_score. ("Entity-type eligibility is not fit" means eligibility is not SUFFICIENT for a high score -- it is never a reason to DOUBT a confirmed-eligible, confirmed-performing client.)

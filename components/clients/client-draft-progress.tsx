...
// The CLIENT soft-launch tile: the IntellEngine panel, present but inert, with a diagonal
// "COMING SOON" watermark. Renders in the dashboard slot ConsoleDraftPanel would occupy, so the
// grid keeps its shape, but carries NO links -- a client cannot navigate into the gated route from
// here (and the route 404s them anyway). Mirrors ConsoleDraftPanel's header so it reads as the same
// feature, just not open yet. Shown to clients only (see ClientDashboard.intellEngineComingSoon);
// staff always get the live panel. The visible "Coming soon to your workspace." line carries the
// state to assistive tech; the rotated watermark is decorative/aria-hidden.

...
// approved · Save for later = pending · Pass = passed. A Pass reason (optional here)
// is routed to the match_feedback calibration store server-side -- this replaced the
// standalone agree/flag score-feedback control that used to sit beside the decision.
  ...
  return (
    <div className="mt-6 border-t border-brand-navy/[0.06] pt-6">
      <div>
        ...
        {/* Pass reason is OPTIONAL for the client, but when given it's routed to the
            match_feedback calibration store server-side (replaced the agree/flag box). */}
        {showPass && !passed && (
          <div className="mt-3 space-y-2 rounded-xl border border-brand-navy/10 bg-brand-cream/50 p-3">
            <p className="text-xs font-medium text-brand-navy">Why pass? (optional — helps us tune your matches)</p>
            <textarea
              value={passReason}

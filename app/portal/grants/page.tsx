...
export default async function PortalGrantReport() {
  ...
  const { data } = await supabase
    .from("review_cards")
    .select(
      "id, grant_id, fit_score, proposed_role, decision, forwarded_to, factor_scores, qa_fit_score, qa_factor_scores, qa_sources, qa_status, qa_engine_fit_score, pursuit_path, client_read_at, grants(title, funder, submission_deadline, award_range_min, award_range_max, award_range_is_estimate, num_awards, total_funding, focus_areas)",
    )

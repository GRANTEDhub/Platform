...
// This is the one-grant test harness for a builder-prompt change -- it deliberately
// does the MINIMUM: no re-fetch, no re-shred, no re-score.
//
...
//   - the seat-stability harness (the verification step) is itself read-only
...
// immediately -- but reading them back via SQL (jsonb) is equally valid.
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (updateError) {
    return NextResponse.json(
      { error: `Write failed: ${updateError.message}` },
      { status: 500 },
    );
  }

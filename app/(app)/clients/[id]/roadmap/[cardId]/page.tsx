...
export default async function ClientRoadmapDetail({ params }: { params: { id: string; cardId: string } }) {
  ...
  const rationale = {
    ...
    // The client-safe narrative REPLACES the three pieces above when present. resolveFit resolves WHICH one:
    // the dedicated fit-analysis narrative (0099) on a fresh go/marginal, else the QA narrative (0088/0090)
    // on a demote/no-go — never both. Null → the assembled paragraph renders as today.
